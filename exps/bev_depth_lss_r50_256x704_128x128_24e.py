...
from layers.discriminator.voxel_disc import Disc_vox_source
from layers.discriminator.voxel_disc import Disc_vox_target
from layers.discriminator.camera_disc import Disc_cam_source
from layers.discriminator.camera_disc import Disc_cam_target
...
class BEVDepthLightningModel(LightningModule):
    ...
    def __init__(self,
                 gpus: int = 1,
                 data_root='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/complete_dataset/nuScenes/',
                 eval_interval=1,
        ...
        self.vox_source_loss = torch.nn.BCEWithLogitsLoss()
        self.vox_target_loss = torch.nn.BCEWithLogitsLoss()
    ...
    #     preds, depth_preds, _, _, _, _ = self(sweep_imgs, mats)
    ...
    def training_step(self, batch, batch_idx):
        # get optimizer
        opt, opt_img_source, opt_img_target, opt_bev_source, opt_bev_target, opt_cam_source, opt_cam_target, opt_vox_source, opt_vox_target = self.optimizers()
        ...
        (sweep_imgs, mats, _, _, gt_boxes, gt_labels, _, depth_labels) = batch["source_train_loader"]
        if torch.cuda.is_available():
            for key, value in mats.items():
                mats[key] = value.cuda()
            sweep_imgs = sweep_imgs.cuda()
        ...
        # vox_domain_label_s = torch.zeros(len(sweep_imgs)*6, requires_grad=True)
        # vox_domain_label_s = vox_domain_label_s.long().cuda()
        cam_domain_label_s = torch.zeros(len(sweep_imgs)*6, requires_grad=True)
        cam_domain_label_s = cam_domain_label_s.long().cuda()
        # patch gan
        bev_domain_label_s = torch.zeros(len(sweep_imgs), 1, requires_grad=True)
        bev_domain_label_s = bev_domain_label_s.float().cuda()
        vox_domain_label_s = torch.zeros(len(sweep_imgs)*6, 1, requires_grad=True)
        vox_domain_label_s = vox_domain_label_s.float().cuda()

        preds, depth_preds, img_feats_source, bev_feats_source, vox_feats_source, cam_feats_source = self(sweep_imgs, mats)

        # training model using target data***************************************    
        (sweep_imgs_t, mats_t, _, _,gt_boxes_t, gt_labels_t, _, depth_labels_t) = batch["target_train_loader"]
        if torch.cuda.is_available():
            for key, value in mats.items():
        ...
        # vox_domain_label_t = torch.ones(len(sweep_imgs_t)*6, requires_grad=True)
        # vox_domain_label_t = vox_domain_label_t.long().cuda()
        cam_domain_label_t = torch.ones(len(sweep_imgs_t)*6, requires_grad=True)
        cam_domain_label_t = cam_domain_label_t.long().cuda()
        # patch gan
        bev_domain_label_t = torch.ones(len(sweep_imgs_t), 1, requires_grad=True)
        bev_domain_label_t = bev_domain_label_t.float().cuda()
        vox_domain_label_t = torch.ones(len(sweep_imgs_t)*6, 1, requires_grad=True)
        vox_domain_label_t = vox_domain_label_t.float().cuda()

        preds_t, depth_preds_t, img_feats_target, bev_feats_target, vox_feats_target, cam_feats_target = self(sweep_imgs_t, mats_t)

        alpha = 1
        # get reverse feature for img and bev feature
        reverse_img_4_source = ReverseLayerF.apply(img_feats_source, alpha)
        reverse_img_4_target = ReverseLayerF.apply(img_feats_target, alpha)
        reverse_bev_4_source = ReverseLayerF.apply(bev_feats_source, alpha)
        reverse_bev_4_target = ReverseLayerF.apply(bev_feats_target, alpha)
        reverse_cam_4_source = ReverseLayerF.apply(cam_feats_source, alpha)
        reverse_cam_4_target = ReverseLayerF.apply(cam_feats_target, alpha)
        reverse_vox_4_source = ReverseLayerF.apply(vox_feats_source, alpha)
        reverse_vox_4_target = ReverseLayerF.apply(vox_feats_target, alpha)
        ...
        # get loss of target domain for test
        if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
            targets_t = self.model.module.get_targets(gt_boxes_t, gt_labels_t)
            detection_loss_t = self.model.module.loss(targets_t, preds_t)
        else:
            targets_t = self.model.get_targets(gt_boxes_t, gt_labels_t)
            detection_loss_t = self.model.loss(targets_t, preds_t)
        if len(depth_labels.shape) == 5:
            # only key-frame will calculate depth loss
            depth_labels_t = depth_labels_t[:, 0, ...]
        depth_loss_t = self.get_depth_loss(depth_labels_t.cuda(), depth_preds_t)
        self.log('target detection_loss', detection_loss_t)
        self.log('target depth_loss', depth_loss_t)

        # adv loss
        source_img_adv_loss = self.img_source_loss(img_out_4_source, domain_label_s)
        target_img_adv_loss = self.img_target_loss(img_out_4_target, domain_label_t)
        source_bev_adv_loss = self.bev_source_loss(bev_out_4_source, bev_domain_label_s)
        target_bev_adv_loss = self.bev_target_loss(bev_out_4_target, bev_domain_label_t)
        source_cam_adv_loss = self.cam_source_loss(cam_out_4_source, cam_domain_label_s)
        target_cam_adv_loss = self.cam_target_loss(cam_out_4_target, cam_domain_label_t)
        source_vox_adv_loss = self.vox_source_loss(vox_out_4_source, vox_domain_label_s)
        target_vox_adv_loss = self.vox_source_loss(vox_out_4_target, vox_domain_label_t)

        # loss backward
        overall_loss = detection_loss + depth_loss + 0.1*(source_img_adv_loss + target_img_adv_loss) + 1*(source_bev_adv_loss + target_bev_adv_loss) + 0.1*(source_cam_adv_loss + target_cam_adv_loss) + 0.1*(source_vox_adv_loss + target_vox_adv_loss)
        ...
        self.log_dict({"loss": overall_loss, "cam_loss": source_cam_adv_loss+ target_cam_adv_loss, "vox_loss": source_vox_adv_loss + target_vox_adv_loss, "det_dep_loss":detection_loss + depth_loss}, prog_bar=True)
    ...
    def eval_step(self, batch, batch_idx, prefix: str):
        (sweep_imgs, mats, _, img_metas, _, _, _) = batch
        if torch.cuda.is_available():
            for key, value in mats.items():
                mats[key] = value.cuda()
            sweep_imgs = sweep_imgs.cuda()
    ...
    def configure_optimizers(self):
        ...
        return [optimizer, optimizer_img_source, optimizer_img_target, optimizer_bev_source, optimizer_bev_target, optimizer_cam_source, optimizer_cam_target, optimizer_vox_source, optimizer_vox_target]
    ...
    def train_dataloader(self):
        train_source_dataset = NuscMVDetDataset(
            ...
            # info_path='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/complete_dataset/nuScenes/nuscenes_12hz_infos_train.pkl',
            is_train=True,
        ...
        train_target_dataset = NuscMVDetDataset(
            ...
            classes=self.class_names,
            data_root='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/BEV-UDA/complete_dataset/nuScenes',
            # info_path='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/BEV-UDA/complete_dataset/nuScenes',
            info_path='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/complete_dataset/nuScenes/nuscenes_12hz_infos_train.pkl',
            is_train=True,
            use_cbgs=self.data_use_cbgs,
            img_conf=self.img_conf,
    ...
    def val_dataloader(self):
        val_source_dataset = NuscMVDetDataset(
            ...
            info_path='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/complete_dataset/nuScenes//nuscenes_12hz_infos_val_night.pkl',
            is_train=False,
            img_conf=self.img_conf,
            num_sweeps=self.num_sweeps,
            sweep_idxes=self.sweep_idxes,
        ...
        val_target_dataset = NuscMVDetDataset(
            ...
            data_root=self.data_root,
            info_path='data/nuScenes/nuscenes_12hz_infos_val.pkl',
            # info_path='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/BEV-UDA/data/nuScenes/nuscenes_12hz_infos_val.pkl',
            is_train=False,
            img_conf=self.img_conf,
            num_sweeps=self.num_sweeps,
            sweep_idxes=self.sweep_idxes,
    ...
    def reload_hyperparameter(self, gpus: int = 1,
                 data_root='/home/notebook/data/group/liujiaming/cvpr2023/rongyu/BEV-UDA/data/nuScenes/',
                 eval_interval=1,
                 batch_size_per_device=8,
                 default_root_dir='./outputs/',
                 **kwargs):
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=16,
        # /outputs/bevuda-sunny-img0.1-bev0.1-cam0.1-vox0.01
        default_root_dir='./outputs/bevuda-fog1-3-aligned')
