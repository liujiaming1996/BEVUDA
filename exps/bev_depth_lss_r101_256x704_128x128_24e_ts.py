...
class BEVDepthLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        self.img_feature_loss = torch.nn.MSELoss()
        self.bev_feature_loss = torch.nn.MSELoss()
        self.vox_feature_loss = torch.nn.MSELoss()
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            pred_mask = (depth_preds_teacher > (torch.mean(depth_preds_teacher)+0.05*torch.std(depth_preds_teacher))) # mean 0.0089 std 0.0468
            ...
            preds_target, depth_preds_target, img_feats_target, bev_feats_target, vox_feats_target, _ = self.teacher(sweep_imgs_target, mats_target, depth_label=sudo_depth)

        preds_target_student, depth_preds_target_student, img_feats_target_student, bev_feats_target_student, vox_feats_target_student, _ = self.student(sweep_imgs_target, mats_target)
        # Student loss
        ...
        bev_loss = self.bev_feature_loss(bev_feats_target,bev_feats_target_student) # too small
        img_loss = self.img_feature_loss(img_feats_target,img_feats_target_student)
        vox_loss = self.vox_feature_loss(vox_feats_target,vox_feats_target_student)

        overall_loss = detection_loss_target + depth_loss_target + detection_loss + depth_loss + 20*(bev_loss + vox_loss + img_loss)
    ...
    def val_dataloader(self):
        ...
        val_target_dataset = NuscMVDetDataset(
            ...
            classes=self.class_names,
            data_root=self.data_root,
            info_path='data/nuScenes/nuscenes_12hz_infos_val_rainy.pkl',
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=16,
        default_root_dir='./outputs/bevuda-fog3-101-final') 
